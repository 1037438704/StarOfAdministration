 ...
 * description : 常用单位的分组
 */
public class LookCompanyAdapter extends BaseQuickAdapter<String, BaseViewHolder> {

    private  int flagRecy = 1;
    private List<String> list;

    public LookCompanyAdapter(int layoutResId, @Nullable List<String> data) {
        super(layoutResId, data);
    }

    public LookCompanyAdapter(int item_message_layout) {
    ...
    protected void convert(BaseViewHolder helper, String item) {
        ...
        LookCompanyGroupAdapter lookCompanyGroupAdapter = new LookCompanyGroupAdapter(R.layout.item_company_list);
        itemGroupRecycle.setAdapter(lookCompanyGroupAdapter);

        list = new ArrayList<>();
        for (int i = 0; i < 15; i++) {
            list.add("" + i);
        }
        lookCompanyGroupAdapter.setNewData(list);
        lookCompanyGroupAdapter.notifyDataSetChanged();
